...
#
# `test` runs --no-parallel: swift-corelibs-foundation UserDefaults loses
# writes under Swift Testing's parallel executor (the known-deferred H7
# test-isolation issue) — serial is deterministic and still sub-second.
#
...
if [ "${1:-}" = "test" ]; then
    MSYS_NO_PATHCONV=1 docker run --rm \
        ...
        bash -c "swift test --no-parallel 2>&1 | tail -3"
else
    MSYS_NO_PATHCONV=1 docker run --rm \
        -v "$HOSTREPO:/src" -w /src swift:6.1 \
